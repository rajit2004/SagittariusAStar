...
/** Weekday narrow names are generated per-locale inside the component. */
      ...
      <section className="glass-card calendar-card">
        <div className="calendar-toolbar">
          <div className="month-nav">
            <button type="button" className="icon-btn" onClick={() => shiftMonth(-1)} aria-label={t('cycle.previousMonth')}>
              ‹
            </button>
            <span className="month-label">{formatMonthYear(displayedMonth)}</span>
            <button type="button" className="icon-btn" onClick={() => shiftMonth(1)} aria-label={t('cycle.nextMonth')}>
              ›
            </button>
          </div>
